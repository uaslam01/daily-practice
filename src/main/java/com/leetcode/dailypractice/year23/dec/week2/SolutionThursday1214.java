...
public class SolutionThursday1214 {
    ...
    public int[][] onesMinusZeros1(int[][] grid) {
        ...
        for(int i=0;i<grid.length;i++){
			for(int j=0;j<grid[i].length;j++) {
				...
				//and similarly no of ones for ith col = total rows of ith col - no of zeros of ith col
				res[i][j]= rowArrOfDiff[i]
						+(grid.length-colArrOfZeros[j])- colArrOfZeros[j];
			}
        }
    ...
    public int[][] onesMinusZeros2(int[][] grid) {
        ...
        for(int i=0;i<grid.length;i++){
			for(int j=0;j<grid[i].length;j++) {
				//no of ones for ith row = total cols of ith row - no of zeros of ith row
				//and similarly no of ones for ith col = total rows of ith col - no of zeros of ith col
				res[i][j]= (grid[i].length-rowArrOfZeros[i])- rowArrOfZeros[i]
						+(grid.length-colArrOfZeros[j])- colArrOfZeros[j];
			}
        }
    ...
    public static void main(String[] args) {
		Consumer cons = i-> Arrays.stream((int[][]) i).forEach( j-> Arrays.stream((int[]) j).forEach(System.out::print));
